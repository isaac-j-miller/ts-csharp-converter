...
export abstract class RegistryType<T extends TokenType> implements IRegistryType<T> {
  ...
  private hashGenericParameters(property: PropertyStructure): string {
    ...
    property.genericParameters?.forEach(g => {
      ...
      if (isGenericReference(g.ref.ref)) {
        gName = g.ref.ref.genericParamName;
      } else if (isConstType(g.ref.ref)) {
        throw new Error("__const__ should not be referenced");
      } else if (isPrimitiveType(g.ref.ref)) {
        gName = g.ref.ref.primitiveType;
      } else {
        const fromRegistry = this.registry.getType(g.ref.ref);
        if (!fromRegistry) {
  ...
  registerRefs(): void {
    ...
    const refsFromProperties = Object.values(properties ?? {}).flatMap(p => [
      ...(p.genericParameters ?? []).flatMap(g => this.extractRefs(g.ref)),
      p.baseType,
