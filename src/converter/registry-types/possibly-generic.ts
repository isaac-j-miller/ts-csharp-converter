...
> extends RegistryType<T> {
  ...
  protected getGenericParametersOfProperty(propName: string): TypeReference[] {
    ...
    if (!isGenericReference(property.baseType)) {
      const baseType = this.registry.getType(property.baseType);
      if (baseType?.isGeneric()) {
        const gParamsToRender = baseType.getGenericParameters();
        const filteredGenericParams = givenGenericParams.filter(g => {
          if (isGenericReference(g.ref.ref)) {
            const asGeneric = g.ref.ref;
            const parentParams = this.getGenericParameters();
            return parentParams.some(gParam => gParam.name === asGeneric.genericParamName);
          }
          return gParamsToRender.some(
            gParam => gParam.name === g.name && baseType.genericParamShouldBeRendered(gParam)
          );
        });
        return filteredGenericParams.map(g => g.ref);
      }
    }
    return [];
  }
